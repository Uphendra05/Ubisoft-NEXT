...
namespace Engine
{
	...
	Entity GamplayUtils::CreateRedUi(CScene* pScene, Vector2 position)
	{
		Entity redBallId = pScene->CreateEntity();
	...
	Entity GamplayUtils::CreateBlueUi(CScene* pScene, Vector2 position)
	{
		Entity redBallId = pScene->CreateEntity();
		...
		SpriteRenderer* pSprite = pScene->AddComponent<SpriteRenderer>(redBallId);
		pSprite->fileName = ".\\Assets\\Blue.png";  
		pSprite->cols = 1;
		pSprite->rows = 1;
		pSprite->animSpeed = 1.0f;
		pSprite->isVisible = true;
